...
**In scope:**
- `src/finpost/posttraining/verifier.py` with three layers:
...
- `src/finpost/posttraining/bucket.py` with the three-bucket assignment (`easy`, `ambiguous`, `hard`) and configurable thresholds.
...
4. `grep -R "anthropic\\|openai" src/finpost/posttraining/verifier.py` returns no matches.
