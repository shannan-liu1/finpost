...
- The same record **schema** (field names and types) is reusable by [`phase1-dpo-comparison`](../../phase1-dpo-comparison/PRD.md), but DPO must build its **own fixed offline preference dataset** — the two workstreams share schema, verifier, and pairwise-loss math, NOT data. The offline-DPO vs. on-policy-OPD split is itself a deliberate comparison axis; sharing data would collapse it.
