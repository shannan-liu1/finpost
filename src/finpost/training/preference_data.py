    ...
    with path.open("r", encoding="utf-8") as fp:
        for line_no, line in enumerate(fp, start=1):
            ...
            try:
                ...
                weight = float(raw.get("weight", metadata.get("online_dpo_weight", 1.0)))
                if weight <= 0.0:
                ...
                raise ValueError(f"{path}:{line_no} invalid preference weight") from exc
    if not examples:
    ...
    def __call__(
        ...
        batch: dict[str, Any] = {
            ...
            "sample_weights": torch.tensor(
                [float(ex.weight) for ex in tokenized],
                dtype=torch.float32,
            ),
        }
        has_ref_logps = [
