...
This module exists to hold one helper - ``token_log_probs_via_cross_entropy``
- and the long explanation of *why* it is written the way it is. Both
the DPO trainer (``finpost.training.dpo``) and the GRPO backend
(``finpost.posttraining.grpo``) need the same quantity: for each
...
def token_log_probs_via_cross_entropy(
    logits: torch.Tensor,
    labels: torch.Tensor,
    *,
    ...
    if logits.size(1) < 2:
        raise ValueError(
            "token_log_probs_via_cross_entropy requires sequence length >= 2"
        )
