...
This module is the repo-native SFT loop: a single ``Trainer(config).train()``
call wires the dataset, optimizer, scheduler, checkpointer, and Weights &
Biases logger together. Everything visible here is thin glue over modules in
this package, with the training loop kept readable instead of hidden behind a
...
DataLoader sampler to checkpoint its index, which is intentionally out of
scope for this small trainer.
"""
...
class Trainer:
    ...
    def _setup(self) -> None:
        ...
        self.model.train()
        # KV-cache off during training: required for gradient checkpointing
        # and a no-op otherwise. We set it unconditionally so the model is
        # in a consistent state regardless of whether checkpointing kicks
        # in below.
        self.model.config.use_cache = False
        if self.config.model.gradient_checkpointing:
            self.model.gradient_checkpointing_enable()
            # When the model has frozen input embeddings (LoRA, or any
            # other partial-freeze pattern), gradients won't flow back to
            # the checkpointed layers without this call. Harmless when
            # everything is trainable, so we set it unconditionally.
    ...
    def _log_train_metrics(self, *, step: int, loss: float, grad_norm: float) -> None:
        ...
        # The optimizer's first param group's lr is the current LR.
        # Both groups always share the same scheduler
