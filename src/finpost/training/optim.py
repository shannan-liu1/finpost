...
def build_8bit_adamw(
    model: nn.Module,
    *,
    lr: float,
    weight_decay: float,
) -> torch.optim.Optimizer:
    """Construct bitsandbytes AdamW8bit with the standard two-group weight-decay split.

    Drop-in replacement for ``build_optimizer`` when VRAM is tight.
    AdamW8bit stores the first and second moments in 8-bit instead of
    fp32, saving roughly ``num_params * 6 bytes`` of GPU memory (e.g.
    ~9 GB on Qwen2.5-1.5B).

    bitsandbytes is imported lazily so the default vanilla-AdamW path
    has no extra dependency. Available in Linux CUDA environments via
    ``pip install -e ".[rlvr]"``.
    """
    try:
        ...
        raise SystemExit(
            "use_8bit_optimizer=True requires bitsandbytes. Install with "
            '`pip install -e ".[rlvr]"` in a Linux CUDA environment.'
        ) from exc
