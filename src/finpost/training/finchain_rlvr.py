"""FinChain helpers for RLVR trainers.

The repo keeps the DPO, online-DPO, GKD, and GRPO math small and testable, but
notebook runs often need a slightly different surface: prompt-only rows for
TRL/Axolotl-style trainers and reward functions that can be passed directly to
GRPO or online-DPO APIs. This module is that adapter layer.
"""
...
def build_finchain_prompt_rows(
    ...
    *,
    ...
    multi_answer_candidates: int | None = None,
) -> list[dict[str, Any]]:
    """Convert FinChain examples into prompt rows for TRL-style trainers."""
    rows: list[dict[str, Any]] = []
    for example in examples:
        prompt = example.prompt
        if multi_answer_candidates is not None:
            if multi_answer_candidates < 2:
                raise ValueError("multi_answer_candidates must be at least 2")
            candidate_tags = "\n".join(
                f"<candidate_{idx}>solution {idx}</candidate_{idx}>"
                for idx in range(1, multi_answer_candidates + 1)
            )
            prompt = (
                f"{prompt}\n\n"
                f"Generate {multi_answer_candidates} distinct solution attempts. "
                "Each attempt must include reasoning and a `Final Answer:` line. "
                "Use exactly these XML-style wrappers:\n"
                f"{candidate_tags}"
            )
        rows.append(
            {
                "prompt": serialize_prompt(prompt) if format_prompt else prompt,
...
def write_jsonl(path: str | Path, rows: Sequence[dict[str, Any]]) -> None:
    """Write JSONL with ASCII-safe encoding for portable GPU artifacts."""
    path = Path(path)
