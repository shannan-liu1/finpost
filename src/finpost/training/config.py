...
# Allowed dataset sources for the repo-native SFT stack. Stored as strings
# so the YAML serializes cleanly as a flat list of literals.
DatasetSource = Literal["finchain"]
...
class ModelConfig(BaseModel):
    ...
    use_safetensors: bool = Field(
        default=True,
        description="Refuse pickle weights; required by SECURITY.md for real model runs.",
    )
    gradient_checkpointing: bool = Field(
        default=False,
        description=(
            "Enable HuggingFace gradient checkpointing on the trainable model. "
            "Trades activation memory for one extra forward per layer; required "
            "when scaling beyond Qwen2.5-1.5B on a single A40."
        ),
    )
    use_8bit_optimizer: bool = Field(
        default=False,
        description=(
            "Use bitsandbytes AdamW8bit instead of fp32 AdamW. Saves ~9 GB of "
            "optimizer state on Qwen2.5-1.5B. Requires the [rlvr] extra."
        ),
    )
    attn_implementation: str | None = Field(
        default=None,
        description=(
            "Attention backend passed to HF `from_pretrained`. None keeps the HF "
            "default (typically `sdpa`). Set to `flash_attention_2` in CUDA environments "
            "with `flash-attn` installed for ~1.5-2x throughput on long sequences."
        ),
    )
...
class DataConfig(BaseModel):
    ...
    sources: list[DatasetSource] = Field(
        ...,
        min_length=1,
        description="Datasets to use for training.",
    )
