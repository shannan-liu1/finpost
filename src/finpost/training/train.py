...
Wires the repo-native SFT training stack behind a single command:

    python -m finpost.training.train --config configs/finchain/sft/local_tiny_gpt2_sft.yaml
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to a training YAML (e.g. configs/finchain/sft/local_tiny_gpt2_sft.yaml).",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main(argv: list[str] | None = None) -> None:
    ...
    ``argv=None`` lets argparse read ``sys.argv`` (the CLI path);
