...
1. load normalized ``Example`` records from the configured source,
...
def serialize_prompt(prompt: str) -> str:
    """Serialize a user prompt in the shared chat template."""
...
class PhasedSFTDataset(Dataset):
    """Torch dataset for supervised fine-tuning examples.

    ``__getitem__`` returns ``(input_ids, prompt_length, source)`` as
    the trainer consumes. Metadata needed for verification, such as
    ``examples`` and ``example_ids``, remains available on the dataset
    object without widening each training item.
    """
