"""Command-line DPO trainer for preference optimization."""
...
class DPOModelConfig(BaseModel):
    ...
    policy_checkpoint: Path = Field(
        ...,
        description="HF-format policy checkpoint to optimize.",
    )
    ...
    gradient_checkpointing: bool = Field(
        default=False,
        description=(
            "Enable HF gradient checkpointing on the policy. Trades activation "
            "memory for one extra forward per layer; required when scaling "
            "beyond Qwen2.5-1.5B on a single A40."
        ),
    )
    use_8bit_optimizer: bool = Field(
        default=False,
        description=(
            "Use bitsandbytes AdamW8bit instead of fp32 AdamW on the policy. "
            "Saves ~9 GB of optimizer state on Qwen2.5-1.5B. Requires [rlvr]."
        ),
    )
    attn_implementation: str | None = Field(
        default=None,
        description=(
            "Attention backend forwarded to HF `from_pretrained`. None preserves "
            "the HF default (typically `sdpa`). Set to `flash_attention_2` on a "
            "CUDA environment with `flash-attn` installed for ~1.5-2x generation throughput."
        ),
    )
...
class DPOTrainer:
    ...
    def _forward_loss(self, batch: dict[str, Any]) -> tuple[torch.Tensor, dict[str, Any]]:
        ...
        attention_mask = torch.cat([chosen_attention_mask, rejected_attention_mask], dim=0)
        pair_count = chosen_input_ids.size(0)
        raw_sample_weights = batch.get("sample_weights")
        sample_weights = (
            raw_sample_weights.to(self.device, non_blocking=True)
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        description="Run Direct Preference Optimization from a YAML config.",
    )
