...
try:
    from scripts.build_dpo_pairs import (
        CompletionRecord,
        build_pair_quality_gate_report,
        quality_thresholds_by_name,
        summarize_completion_groups,
        summarize_pair_quality,
        summarize_pair_rows,
        write_pair_review_sample,
    )
...
def _validate_manifest_invariants(manifests: list[dict[str, Any]]) -> None:
    first = manifests[0]
    for index, manifest in enumerate(manifests[1:], start=1):
        for field in INVARIANT_MANIFEST_FIELDS:
            if manifest.get(field) != first.get(field):
                raise ValueError(
                    "shard manifests disagree on "
                    f"{field}: shard 0 has {first.get(field)!r}, "
                    f"shard {index} has {manifest.get(field)!r}"
                )
...
def merge_shards(
    *,
    ...
    _validate_manifest_invariants(manifests)
    ...
    review_sample_path = out_dir / "pair_review_sample.csv"
    ...
    first = manifests[0]
    write_pair_review_sample(review_sample_path, pairs, seed=int(first.get("seed") or 42))
    ...
    thresholds = quality_thresholds_by_name(quality_gate_name)
    quality_gate = None
    if thresholds is not None:
        quality_gate = build_pair_quality_gate_report(pair_quality, thresholds=thresholds)
    manifest = {
        "created_at": datetime.now(UTC).isoformat(),
        "merge_of": [str(Path(path).resolve()) for path in shard_dirs],
        "model_checkpoint": first.get("model_checkpoint"),
        "model_checkpoint_fingerprint": first.get("model_checkpoint_fingerprint"),
        "verifier": first.get("verifier"),
        "sources": first.get("sources"),
        "data_paths": first.get("data_paths"),
        "data_checksums_sha256": first.get("data_checksums_sha256"),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="none",
        help="Optional DPO pair quality gate to enforce after merging.",
    )
...
def main() -> None:
    ...
    manifest = merge_shards(
        ...
        quality_gate_name=args.quality_gate,
    )
